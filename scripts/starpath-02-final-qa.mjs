...
async function run() {
  ...
  try {
    ...
    checks.nodeDetailOpens = true;

    await page.getByText('Explore', { exact: true }).click({ timeout: 5000 });
    await page.waitForTimeout(400);
    checks.exploreWorks = (await page.locator('[data-testid="starpath-node-detail"]').count()) === 0;

    await node.click({ timeout: 8000 });
    await page.waitForSelector('[data-testid="starpath-node-detail"]', { timeout: 8000 });
    await page.getByText('Interested', { exact: true }).click({ timeout: 5000 });
    await page.waitForTimeout(400);
    checks.interestedWorks = true;

    await node.click({ timeout: 8000 });
    await page.getByText('Save for later', { exact: true }).click({ timeout: 5000 });
    await page.waitForTimeout(400);
    checks.saveWorks = true;

    await node.click({ timeout: 8000 });
    await page.getByText('Not for me', { exact: true }).click({ timeout: 5000 });
    await page.waitForTimeout(400);
    checks.dismissWorks = true;

    await node.click({ timeout: 8000 });
    await page.getByText('Bring back', { exact: true }).click({ timeout: 5000 });
    await page.waitForTimeout(400);
    checks.reversibleDismiss = true;

    await page.locator('[data-testid="journey-traveler"]').click({ timeout: 8000 });
    await page.waitForSelector('[data-testid="starpath-avatar-sheet"]', { timeout: 8000 });
    checks.avatarSheetOpens = true;

    await page.getByText('Use profile photo').click({ timeout: 5000 });
    await page.waitForTimeout(500);
    checks.profilePhotoPath = (await page.locator('[data-testid="starpath-avatar-sheet"]').count()) === 0;

    await page.locator('[data-testid="journey-traveler"]').click({ timeout: 8000 });
    await page.waitForSelector('[data-testid="starpath-avatar-sheet"]', { timeout: 8000 });
    await page.getByText('Dawn', { exact: true }).click({ timeout: 5000 });
    await page.waitForTimeout(500);
    checks.presetAvatar = true;

    await page.locator('[data-testid="journey-traveler"]').click({ timeout: 8000 });
    await page.getByText('Use custom avatar', { exact: true }).click({ timeout: 5000 });
    await page.waitForTimeout(500);
    checks.customAvatar = true;

    const storageKeys = await page.evaluate(() => Object.keys(localStorage));
    checks.interactionStorage = storageKeys.some((k) => k.includes('starpath-interactions'));
    checks.avatarStorage = storageKeys.some((k) => k.includes('user-avatar-identity'));
    ...
    const pass =
      checks.routeLoads &&
      checks.nodeDetailOpens &&
      checks.exploreWorks &&
      checks.interestedWorks &&
      checks.saveWorks &&
      checks.dismissWorks &&
      checks.reversibleDismiss &&
      checks.avatarSheetOpens &&
      checks.profilePhotoPath &&
      checks.presetAvatar &&
      checks.customAvatar &&
      checks.interactionStorage &&
      checks.avatarStorage &&
      checks.reloadStable &&
      checks.nextStepCollapsed &&
