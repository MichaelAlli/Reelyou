...
export function FourPointStar({
  ...
  const inner = size * CelestialStarGeometry.fourPointInnerRatio;
  const starPath = `
    M ${cx} ${cy - size}
    L ${cx + inner} ${cy - inner}
    L ${cx + size} ${cy}
    ...
    <G opacity={opacity} transform={rotateTransform}>
      <Defs>
        <RadialGradient id={`${id}-4`} cx="38%" cy="30%" r="72%">
          <Stop offset="0%" stopColor={CelestialPalette.warmWhiteCore} stopOpacity={1} />
      ...
      <Line
        ...
        x2={cx + size * 1.4}
        y2={cy}
        stroke={CelestialPalette.warmWhite}
        strokeWidth={0.32}
        opacity={CelestialStarGeometry.fourPointCrossRayOpacity}
        strokeLinecap="round"
      />
      <Line
        x1={cx}
      ...
      <Circle cx={cx} cy={cy} r={Math.max(0.4, size * 0.14)} fill={CelestialPalette.warmWhiteCore} opacity={0.96} />
    </G>
  );
}

/** Compact fill-only glyph for animated flying / landing stars (preserves transition look). */
export function CompactFourPointStar({ size, color }: { size: number; color: string }) {
  const half = size / 2;
  const inner = size * CelestialStarGeometry.fourPointCompactInnerRatio;
    ...
    <G opacity={opacity}>
      <Defs>
        <RadialGradient id={`${id}-6`} cx="50%" cy="50%" r="50%">
          <Stop offset="0%" stopColor={CelestialPalette.warmWhiteCore} stopOpacity={0.92} />
...
export function PremiumStar({
  ...
  const inner = size * CelestialStarGeometry.premiumInnerRatio;
  const starPath = `
    M ${cx} ${cy - size}
    L ${cx + inner} ${cy - inner}
    L ${cx + size} ${cy}
    ...
    Z
  `;
  const halo = size * (CelestialStarGeometry.premiumHaloRatio + intensity * CelestialStarGeometry.premiumHaloIntensityBoost);
    ...
    <G>
      <Defs>
        ...
        <RadialGradient id={`${id}-c`} cx="40%" cy="34%" r="70%">
          <Stop offset="0%" stopColor={CelestialPalette.warmWhiteCore} stopOpacity={1} />
          <Stop offset="35%" stopColor={CelestialPalette.warmWhite} stopOpacity={0.98} />
      ...
      <Line
        ...
        stroke={CelestialPalette.warmWhite}
        strokeWidth={0.4}
        opacity={CelestialStarGeometry.premiumCrossRayOpacity * intensity}
        strokeLinecap="round"
      />
      <Line
        x1={cx}
      ...
      <Circle cx={cx} cy={cy} r={Math.max(0.55, size * 0.16)} fill={CelestialPalette.warmWhiteCore} opacity={0.98} />
    </G>
  );
}

export const GlowingStar = FourPointStar;
export const HeroStar = PremiumStar;
...
export function ConstellationLine({
  ...
  const preset = CelestialConstellationStroke.ambient;
  return (
    <Line
      x1={x1}
      y1={y1}
    ...
    <G>
      <Defs>
        ...
        <RadialGradient id={`${id}-fill`} cx="34%" cy="28%" r="76%">
          <Stop offset="0%" stopColor={CelestialPalette.warmWhiteCore} stopOpacity={1} />
          <Stop offset="18%" stopColor={CelestialPalette.warmWhite} stopOpacity={0.97} />
      ...
      <Circle cx={cx - r * 0.22} cy={cy - r * 0.28} r={r * 0.22} fill={CelestialPalette.warmWhiteCore} opacity={0.38} />
      {children}
    </G>
  );
}
...
export function DustStar({
  ...
  opacity = 0.4,
  size = 0.65,
  color = CelestialPalette.lavender,
}: {
  cx: number;
  cy: number;
  opacity?: number;
  ...
  return (
    <G opacity={opacity}>
      ...
      <Circle cx={cx} cy={cy} r={size * 0.48} fill={CelestialPalette.warmWhiteCore} opacity={0.88} />
    </G>
  );
}
